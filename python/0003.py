...
1. 파일을 입력받아 파일에 가장 많이 쓰인 단어와 그 횟수를 출력하는 프로그램
...
for word, cnt in word_count.most_common(1): # word_count에서 Counter가 가장 높은 단어 1개(most_common(1))와 횟수를 가져온다.
    print("가장 많이 쓰인 단어와 그 횟수\n"+word,"=>", cnt) # 가져온 단어(word)와 횟수(cnt)를 출력한다.
